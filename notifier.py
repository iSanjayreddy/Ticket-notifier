...
# ================== CONFIG ==================
TARGET_DATE = "2026-04-11"          # Change this date when needed
...
print(f"🎟️ Checking for Project Hail Mary IMAX on {TARGET_DATE} (Chennai)...")
...
try:
    ...
    # Check if real show data appeared
    ...
    if discovery.get("data") or (sessions and sessions != {}):
        send_notification(
            ...
            f"Date: {TARGET_DATE}\n"
            f"Chennai → Book immediately on District!"
        )
        print("✅ IMAX shows found!")
    else:
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Still no IMAX shows yet.")

except Exception as e:
    error_msg = f"⚠️ Check failed: {str(e)[:150]}"
    print(error_msg)
    send_notification(error_msg)

print("✅ Single check completed. GitHub will run this again in 5 minutes.")
